...
function initSocket() { // 初始化 socket.io 连接，一个socket只需要配置一次
  ...
  //socket = io("http://localhost:3000"); // 本地测试时使用
    ...
    socket.on("room-created", (roomId) => {
      ...
      winRef.document.write(`<p>等待匹配对手...你的房间号是：${roomId}，可以发给对手。</p>`); // 在新窗口中显示等待信息
    });
    ...
    socket.on("start-game", (msg) => {
      ...
      all_sid = msg.players;  // 获取所有玩家的 socket ID
      if (all_sid[0] === socket.id) {
        oppsid = all_sid[1];  // 对手的 socket ID
      } else {
